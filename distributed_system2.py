...
# Global set to keep track of worker processes
worker_processes: Set[subprocess.Popen] = set()

def cleanup():
    """Terminate all worker processes on exit"""
...
class WorkerManager:
    ...
    def _start_local_worker(self, worker: Dict) -> bool:
        ...
        try:
            ...
            # Start the process and add it to our tracking set
            ...
            print(f"Started local worker: {worker['worker_id']} (PID: {process.pid})")
    ...
    def _start_remote_worker(self, worker: Dict) -> bool:
        ...
        try:
            ...
            if os.path.exists(remote_exe):
                ...
                worker_processes.add(process)
                print(f"Started remote worker: {worker['worker_id']} (PID: {process.pid})")
                worker['status'] = 'running'
                self._save_workers()
                return True
