...
def main():
    ...
    excel_path = os.path.join(base_dir, EXCEL_FILENAME)  # Path to input Excel file
    config_path = os.path.join(base_dir, CONFIG_FILENAME)  # Path to config file
    ...
    with ProcessPoolExecutor(max_workers=n_cores) as executor:
        ...
        while next_to_submit < min(n_cores, len(pending_batches)):
            ...
            case_id = batch_inputs[11] if len(batch_inputs) > 11 and batch_inputs[11] else f"UNKNOWN_{row_idx}"
            print(f"[INFO] CaseID {case_id} executing on Core {core_no + 1}")
