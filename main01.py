...
import traceback             # For error detection
...
def get_base_path():
    ...
    Returns the directory containing the executable when frozen, or the script directory when running as script.
    """
...
def get_absolute_path(relative_path):
    ...
    Raises:
        FileNotFoundError: If the file doesn't exist at the resolved path
    """
    ...
    if not abs_path.exists():
        ...
        print(f"[DEBUG] Files in directory: {os.listdir(base_path)}")
...
def run_exe_on_batch(inputs, exe_name):
    ...
    """
    try:
        # Resolve the executable path using our path resolution
        exe_full_path = str(Path(get_base_path() / exe_name).resolve())
        
        # Verify the executable exists before trying to run it
        if not os.path.exists(exe_full_path):
            error_msg = f"Executable not found: {exe_full_path}"
        ...
        print(f"[DEBUG] Executing: {' '.join(args)}")
        ...
        result = subprocess.run(
            ...
            check=True,
            text=True,
            cwd=get_base_path()  # Set working directory to the executable's directory
        )
...
def read_output_file(case_id):
    ...
    Args:
        case_id (str): The ID of the case to read output for
        
    Returns:
        tuple: (content, success)
            - content (str): The content of the output file, or empty string if not found
            - success (bool): True if the file was read successfully, False otherwise
    """
    try:
        # Resolve the output file path relative to the base directory
        output_filename = f"Output{case_id}.txt"
        output_path = Path(get_base_path()) / output_filename
        ...
        # Log successful read (without the actual content to avoid cluttering logs)
        print(f"[DEBUG] Successfully read output file: {output_path} ({len(content)} characters)")
        
        return content, True
        
    except Exception as e:
        error_msg = f"Failed to read output file {output_path}: {str(e)}"
        print(f"[ERROR] {error_msg}")
        print(f"[TRACEBACK] {traceback.format_exc()}")
        return "", False


def process_batch(row_idx, batch_inputs, exe_name, core_no, case_id=None):
    """
    ...
    Args:
        ...
        core_no (int): The CPU core number this batch is assigned to (0-based index).
    ...
    Returns:
        tuple: A 7-element tuple containing:
            ...
            - core_used (str): Formatted string indicating which core was used (1-based)
...
def kill_excel():
    """Forcefully close all Excel processes."""
    import subprocess
    import platform
    import time
    ...
    try:
        ...
        # Give the system a moment to close Excel
        time.sleep(2)
        return True
    except Exception as e:
...
def show_popup_message(time_remaining=None):
    """Show a popup message asking to close the file."""
    try:
        ...
        # Prepare the message
        message = "The Excel file is open. Please close it to continue."
        if time_remaining is not None:
            minutes = time_remaining // 60
            seconds = time_remaining % 60
            message += f"\nForce closing Excel in {minutes}m {seconds:02d}s..."
        
        # Show the message box
        messagebox.showwarning("File Open", message)
        
        # Clean up
        root.destroy()
    except Exception as e:
        # Fallback to console message if tkinter fails
        if time_remaining is not None:
            minutes = time_remaining // 60
            seconds = time_remaining % 60
            print(f"The Excel file is open. Force closing in {minutes}m {seconds:02d}s...")
        else:
            print("The Excel file is open. Please close it to continue.")

def wait_excel(filepath):
    """
    Wait for an Excel file to be closed by the user.
    
    This function will:
    1. Wait for the file to be available
    2. Show a popup if the file is locked
    3. After 1 minute, force close Excel if the file is still locked
    ...
    while True:
        try: 
            ...
            with open(filepath, 'a'):
                if popup_shown:  # Only print if we were waiting
                    print("File is now available for writing")
                break  # Exit the loop when file is available
            
        except IOError:
            current_time = time.time()
            elapsed = current_time - start_time
            time_remaining = max(0, timeout - int(elapsed))
            
            # Show popup with countdown
            if not popup_shown or (time_remaining % 30 == 0):  # Update countdown every 30 seconds
                show_popup_message(time_remaining)
                popup_shown = True
                
                # If timeout reached, try to kill Excel
                if elapsed >= timeout:
                    print("Timeout reached. Attempting to close Excel...")
                    if kill_excel():
                        print("Excel has been force closed.")
                        time.sleep(2)  # Give system time to close Excel
                        continue  # Try opening the file again
                    else:
                        print("Could not close Excel. Please close it manually.")
...
def main():
    ...
    # 1. INITIALIZATION PHASE
    # ============================================
    # 1.1 Get the absolute paths to required files
    try:
        excel_path = get_absolute_path(EXCEL_FILENAME)
        config_path = get_absolute_path(CONFIG_FILENAME)
        
        # 1.2 Load configuration and determine system resources
        ...
        # Resolve the executable path relative to the config file location
        exe_name = str(Path(get_base_path() / exe_name).resolve())
        
    except FileNotFoundError as e:
        print(f"[ERROR] {e}")
        print(f"[DEBUG] Current working directory: {os.getcwd()}")
        print(f"[DEBUG] Base path: {get_base_path()}")
        print(f"[DEBUG] Files in directory: {os.listdir(get_base_path())}")
        sys.exit(1)
    ...
    with ProcessPoolExecutor(max_workers=n_cores) as executor:
        ...
        while next_to_submit < min(n_cores, len(pending_batches)):
            ...
            wb.save(excel_path)  # Save after each update for persistence
            ...
            fut = executor.submit(process_batch, row_idx, batch_inputs, exe_name, core_no, case_id)
        ...
        while futures:
            ...
            for fut in done:
                ...
                try:
                    ...
                    #         - core_str: Which core processed this batch
                    #         - full_output: Tab-separated output data
                    #         - error_msg: Error message if any
                    r_idx, status, start, end, core_str, full_output, error_msg = fut.result()
                except Exception as e:
                    ...
                    wb.save(excel_path)  # Save error status
                ...
                if full_output:
                    ...
                    for field_idx, field in enumerate(output_fields):
                        # Calculate target column (status_col + 3 skips status, start, end columns)
